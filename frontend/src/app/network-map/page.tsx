...
export default function NetworkMap() {
  ...
  const fetchDevices = async () => {
    try {
      ...
      if (res.ok) { const data: Device[] = await res.json(); setDevices(data); generateTopology(data); }
    } catch (error) { console.error(error); } finally { setLoading(false); }
  };

  const generateTopology = (devicesList: Device[]) => {
  ...
  const onNodeClick = (_event: React.MouseEvent, node: Node<NodeData>) => { setSelectedNode(node); setTimeout(() => setSelectedNode(null), 4000); };
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="bg-slate-800/20 rounded-xl border border-slate-600/70 overflow-hidden shadow-2xl"><div style={{ height: 'calc(100vh - 320px)', minHeight: '500px' }}>
          <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onNodeClick={onNodeClick} nodeTypes={nodeTypes} fitView attributionPosition="bottom-right">
            ...
            <MiniMap className="bg-[#121a2b] border border-slate-600/70 rounded-lg shadow-lg" nodeColor={(node) => { if (node.id === 'core') return '#3b82f6'; return (node.data as NodeData)?.status === 'online' ? '#10b981' : '#ef4444'; }} maskColor="rgba(0, 0, 0, 0.6)" />
          </ReactFlow>
        </div></div>
