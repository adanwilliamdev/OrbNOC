...
// Rota de saúde - Para monitoramento (Uptime Robot, Render Health Check)
...
// Rota de diagnóstico rápido (para debug)
...
app.post('/api/diagnostic/full-diagnostic', authenticateToken, async (req, res) => {
  ...
  for (const port of portResults) {
    diagnosis.push(port.open ? `✅ Porta ${port} aberta` : `❌ Porta ${port} fechada`);
  }
    ...
    for (const device of devices) {
      try {
        ...
        if (previousStatus && previousStatus !== newStatus) {
          ...
          if (user && user.telegram_alerts_enabled && user.telegram_bot_token && user.telegram_chat_id) {
            let alertType, extraInfo;

            if (newStatus === 'offline') {
              alertType = 'error';
              extraInfo = `📊 *Status:* OFFLINE`;
            } else {
              alertType = 'success';
              extraInfo = `📊 *Status:* ONLINE\n⚡ *Latência:* ${latency || 'N/A'}ms`;
            }
            ...
            await sendTelegramAlert(
              ...
              user.telegram_chat_id,
              newStatus === 'offline'
                ? `Host não está respondendo aos testes.`
